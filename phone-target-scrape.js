...
async function scrapeProduct(url, sixFigs) {
    ...
    for (let i=0; i < sixFigs.length; i++) {
        ...
        // file update -- better
        fs.appendFile('hisphonenums.txt', JSON.stringify({sixFig, value})+'\n', function (err) {
            if (err) throw err;
...
var godfArray = fs.readFileSync('tels.txt').toString().split("\n").slice(750, )
const isNumber = (el) => el === '000742'
console.log(godfArray.findIndex(isNumber))

// run W TARGET
scrapeProduct('https://www.globaltel.rs/smisli-broj', godfArray)
